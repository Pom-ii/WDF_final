            ...
            if (error) {
              ...
              bcrypt.compare(password, cell.upassword, (err, result) => {
                ...
                if (result) {
                  ...
                  //user input is not the admin password
                  const model = {
                    error: "Wring password, I'm afraid...",
                    message: "",
                  };
                  res.status(400).render("login", model);
                }
...
app.post("/user/modify/:userid", (req, res) => {
  ...
  //specific record in games table is updated with new information (UPDATE)
  db.run(
    "UPDATE users SET uname=?, upassword=? WHERE uid=?",
    [name, password, id],
    (error) => {
      if (error) {
        //error log
...
app.get("/users", (req, res) => {
  if (req.session.isAdmin) {
    //list all records from users table & render users page (READ)
...
app.get("/user/delete/:userid", (req, res) => {
  if (req.session.isAdmin) {
    // (DELETE) delete specific user from users table & render users page
